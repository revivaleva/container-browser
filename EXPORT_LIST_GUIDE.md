...
Kameleo 版 Container Browser では、コンテナごとに Kameleo プロファイルが紐付けられています。

### 1. プロファイル管理モード (`profileMode`)
リストに含まれるコンテナがどのようなモードで動作するかを確認してください：
- **`managed`**: コンテナとライフサイクルをともにする自動生成プロファイル。
- **`attached`**: 外部で作成・管理されているプロファイルをコンテナが「借用」している状態。

### 2. 環境指定 (`environment`)
コンテナ作成 API (`/internal/containers/create`) で `environment` を指定した場合、その指定内容は DB に保存され、エクスポート対象にも含まれます。移行先で再作成する際、同じ `deviceType`, `os`, `browser` 設定が引き継がれます。

### 3. Single-tab 環境の運用
本システムは Single-tab 運用を前提としています。リスト管理されるコンテナは、常に単一のブラウザウィンドウおよびメインページとして操作されることを期待してください。

## 関連ドキュメント

- `EXPORT_GUIDE.md` - エクスポート手順
- `EXPORT_PROGRESS_GUIDE.md` - 進捗表示ガイド
- `README.md` - プロジェクト概要と API 仕様
