...
- `POST /internal/containers/create`: コンテナ作成。
  - `environment`: `{ deviceType, os, browser }` を指定して、条件に合う指紋でプロファイルを新規生成。
- `POST /internal/containers/{id}/attach`: 既存のプロファイル ID をコンテナに紐付け。
  - 指定された `profileId` が実在することを検証し、メタデータをキャッシュします。
- `POST /internal/containers/{id}/detach`: 紐付けを解除。コンテナは `managed` モードに戻ります。

### 実行操作 (`/internal/exec`)
- 従来のナビゲーション、クリック、タイピング、評価などは Playwright (CDP) 経由で実行されますが、呼び出し形式は旧版と概ね互換性が維持されています。
