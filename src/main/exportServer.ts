...
async function solveCaptcha(wc: WebContents, options: any): Promise<{ ok: boolean; token?: string; type?: string; action?: string; error?: string; errorDetail?: any }> {
  ...
  let info: any;
  try {
    info = await wc.executeJavaScript(extractionScript, true);
  } catch (e: any) {
  ...
  const pageUrl = options.url || wc.getURL();
  ...
  try {
    ...
    while (Date.now() - start < timeoutMs) {
      ...
      if (resResult.status === 1) {
        ...
        const actionTaken = await wc.executeJavaScript(injectionScript, true);
        ...
        try {
          ...
          try {
            ...
            if (command === 'save_media') {
              ...
              const solveResult = await solveCaptcha(wc, options);
              if (solveResult.ok) {
